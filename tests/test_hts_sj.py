...
from pi_atlas.methods.hts_generic import fit_hts_sj
from pi_atlas.validation.r_bridge import metafor_hts_sj
...
def test_hts_sj_matches_metafor_mu_hat(simple_ma):
    y, v = simple_ma
    py = fit_hts_sj(y, v)
    r = metafor_hts_sj(y, v)
    assert py["mu_hat"] == pytest.approx(r["mu_hat"], abs=1e-5)


def test_hts_sj_matches_metafor_tau2(simple_ma):
    y, v = simple_ma
    py = fit_hts_sj(y, v)
    r = metafor_hts_sj(y, v)
...
def test_hts_sj_matches_metafor_pi(simple_ma):
    y, v = simple_ma
    py = fit_hts_sj(y, v)
    r = metafor_hts_sj(y, v)
